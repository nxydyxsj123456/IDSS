...
class CVDD(object):
    ...
    def set_network(self, net_name, dataset,vocab_size, pretrained_model=None, embedding_size=None, attention_size=150,
                    n_attention_heads=3):
        """Builds the CVDD network composed of a pretrained_model, the self-attention module, and context vectors."""
        self.net_name = net_name
        self.net = build_network(net_name, dataset,vocab_size, embedding_size=embedding_size, pretrained_model=pretrained_model,
                                 update_embedding=False, attention_size=attention_size,
                                 n_attention_heads=n_attention_heads)

    def train(self, dataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 25,
    ...
    def test(self,normal_dataset,anomalous_dataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        """Tests the CVDD model on the test data."""

        if self.trainer is None:
            self.trainer = CVDDTrainer(device, n_jobs_dataloader)

        self.trainer.test(normal_dataset,anomalous_dataset, self.net, ad_score=self.ad_score)
