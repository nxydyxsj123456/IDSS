...
class CVDDTrainer(BaseTrainer):
    ...
    def train(self, dataset, net: CVDDNet):
        ...
        train_loader = DataLoader(dataset,batch_size=self.batch_size, num_workers=self.n_jobs_dataloader,shuffle=False)
    ...
    def test(self, normal_dataset, anomalous_dataset, net: CVDDNet, ad_score='context_dist_mean'):
        logger = logging.getLogger()
        ...
        normal_test_loader = DataLoader(normal_dataset,batch_size=self.batch_size, num_workers=self.n_jobs_dataloader,shuffle=False)
        anomalous_test_loader = DataLoader(anomalous_dataset,batch_size=self.batch_size, num_workers=self.n_jobs_dataloader,shuffle=False)
        ...
        with torch.no_grad():
            for data in normal_test_loader:
                ...
                text_batch = text_batch.to(self.device)
                text_batch = text_batch.transpose(0, 1)
                label_batch= torch.zeros(64,dtype=torch.float).to(self.device)#  label_batch.to(self.device)
                ...
                idx_label_score_head += list(zip(#TODO idx,
                                                 label_batch.cpu().data.numpy().tolist(),
                                                 ad_scores.cpu().data.numpy().tolist(),
                                                 best_att_head.cpu().data.numpy().tolist()))
               #TODO att_weights += A[range(len(idx)), best_att_head].cpu().data.numpy().tolist()
                ...
                n_batches += 1
            #######################################
            for data in anomalous_test_loader:
                idx=1  #TODO
                text_batch= data
                text_batch = text_batch.to(self.device)
                text_batch = text_batch.transpose(0, 1)
                label_batch= torch.ones(64,dtype=torch.float).to(self.device)#  label_batch.to(self.device)
        ...
        # Compute AUC
        labels, scores, _ = zip(*idx_label_score_head)
        ...
        # Get top words per context
        # TODO self.test_top_words = get_top_words_per_context(dataset.test_set, dataset.encoder, net, test_loader,
        #                                                self.device)
...
def initialize_context_vectors(net, train_loader, device):
    ...
    X = ()
    for data in train_loader:
        text = data # { 'weight', 'text', 'index','label'}
        text = text.to(device)
    ...
    kmeans = KMeans(n_clusters=n_attention_heads,random_state=25).fit(X)   #聚成三类
